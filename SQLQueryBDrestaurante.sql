...
alter table RESERVACION
   add constraint FK_RESERVAC_RELATIONS_CLIENTES foreign key (id_clientesfk3)
      references CLIENTES (id_clientes)
go
select * from RESERVACION
...
select *from HORARIO_MESERO

INSERT INTO HORARIO_CHEF(id_horario,id_cheffk2,id_menufk,fecha_entrada,fecha_salida,hora_entrada,hora_salida) 
VALUES (1,1,1,'2020-05-12','2021-05-13','15:00','23:00'),
        (2,2,1,'2020-05-14','2021-05-15','15:00','23:00'),
		(3,3,1,'2020-05-16','2021-05-1','15:00','23:00')
select *from HORARIO_CHEF		
...
VALUES   
        (3,3,'20:00','2021-02-20','pareja' ,22.05,'aceptada'),
		(4,4,'19:00','2021-02-22','familiar' ,15.35,'aceptada'),
		(1,1,'19:00','2021-02-22','familiar' ,22.05 ,'aceptada'),
		(2,2,'19:00','2021-02-22','familiar' ,22.05,'aceptada'),
		(5,1,'21:00','2021-02-22','familiar' ,22.05,'cancelada'),
		(6,1,'19:00','2021-02-22','pareja' ,22.05,'cancelada')
		select * from RESERVACION



INSERT INTO PEDIDO_CLIENTE(id_pedido,id_clientesfk2,id_meserofk,descripcion,costo,fecha_pedido) 
VALUES   (6,1,1,'poca sal',10.50,'2021-02-22'),
	    (5,1,3,'completo',10.00,'2021-03-27'),
        (4,3,2,'completo',10.00,'2021-02-24'),
		(2,1,1,'poco arroz',7.50,'2021-02-23'),
		(3,2,1,'completo',7.50,'2021-02-23'),
       (1,1,1,'plato sin cevolla',10.50,'2021-02-22')
...
VALUES (1,1,'Apanado de pezcado','receta sencilla','1:00','5',2,2020),
		...
		(3,2,'carne asada','receta sencilla','2:00','5',3,2021),
...
--trigger normal
create TRIGGER TG_productoinsertado 
on CLIENTES after insert
AS
print 'datos eliminados correctamente'
go

create TRIGGER TG_operaciones 
on CLIENTES after insert, delete, update
AS
print 'datos siendo violados'
go


DROP TRIGGER IF EXISTS TG_productoinsertado


INSERT INTO CLIENTES(id_clientes,Cedula, Nombres, Apellidos, telefono,correo,direccion) 
VALUES (11,'12345356', 'Damian', 'Rojo','097631568', 'Rojo@gmail.com' ,'Barrio lo sangeles')
...
create trigger trventas
on PEDIDO_CLIENTE 
FOR INSERT
AS 
set nocount on
update PLATOS set PLATOS.existencia = PLATOS.existencia - inserted.cantidad from inserted
inner join PLATOS on PLATOS.id_platos = inserted.id_pedido
print 'valio'
go

DROP trigger trventas